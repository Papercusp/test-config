 ...
 * `vitest-config.ts` (or `pg.ts`) and asserts the warning never fires. It also
 * spawns one importing the full barrel (`index.ts`) and asserts the warning
 * DOES fire there — proving the test would actually catch a regression (e.g.
 * someone re-adding a heavy re-export directly to vitest-config.ts/pg.ts)
 * rather than trivially passing regardless of what's imported.
 */
...
describe('the lightweight @papercusp/test-config/vitest-config subpath never triggers the msw/localStorage warning (EI-13226)', () => {
  ...
  it('importing pg.ts alone prints NO --localstorage-file warning', () => {
    const stderr = stderrFromFreshNodeImport('./pg.ts');
    expect(stderr).not.toMatch(LOCALSTORAGE_WARNING);
  });
});
